...
```
객체가 detach 되었거나 em.clear() 된 상태에서 프록시 초기화를 시도하면 에러가 발생한다.   
(em detach대신 em.close하면 에러가 발생하지 않는다.)   
👍결론 : 같은 session과 thread여야 하고 영속상태고 영속성 컨텍스트안에 프록시 객체가 있어야 정상 작동한다.   
